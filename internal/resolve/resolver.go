...
// New constructs a Resolver bound to the given HTTP client. The client may be
// nil for tests that only exercise the URN-fallback path.
func New(c *client.Client) *Resolver {
	return &Resolver{client: c, ttl: defaultTTL, cache: map[string]entry{}}
}
