...
func TestAPIError_403_WithScope(t *testing.T) {
	t.Parallel()
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusForbidden)
		_, _ = w.Write([]byte(`{"status":403,"code":"FORBIDDEN","message":"Not enough permissions: scope: rw_ads","serviceErrorCode":500}`))
	}))
	defer srv.Close()
	...
	msg := apiErr.Error()
	if !strings.Contains(msg, "missing scope: rw_ads") {
		t.Errorf("expected 'missing scope: rw_ads' in: %s", msg)
	}
	if !strings.Contains(msg, "serviceErrorCode: 500") {
		t.Errorf("expected 'serviceErrorCode: 500' in: %s", msg)
	}
}

func TestAPIError_403_NoScope(t *testing.T) {
	t.Parallel()
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusForbidden)
		_, _ = w.Write([]byte(`{"status":403,"code":"FORBIDDEN","message":"access denied"}`))
	}))
	defer srv.Close()
	...
	if !strings.Contains(apiErr.Error(), "missing the required scope") {
		t.Errorf("expected generic scope hint, got: %v", apiErr)
	}
}
